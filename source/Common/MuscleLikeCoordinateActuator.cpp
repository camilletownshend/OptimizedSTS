...
double MuscleLikeCoordinateActuator::computeActuation( const SimTK::State& s ) const
{
	...
	// getOptimalForce() will return the "gain" defined for this actuator. It can be assumed to just equal zero.
	return getControl(s) * getOptimalForce() * getForceVsCoordinateFunctionValue(s) * getVelocityMultiplier(s);
	//getControls: asks state what the controls are. Controls are known in the forward sim, which is where this works.
	//getForceVsCoordinateFunctionValues: Function that defines the optimal force curve vs. coordinate with positive/negative activation.
	//getVelocityMultiplier: scaling factor representing the dependence of the torque on the angular velocity of the joint.
}
